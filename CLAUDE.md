...
> **CURRENT: Step 6 — oversampling (juce::dsp::Oversampling, 8x pref) + 1st-order ADAA on the nonlinear SW1 clipping stage only. Step 5 (SW1 clipping) complete & validated: diodes live INSIDE Stage 1's feedback (folded into `src/dsp/Stage1.h`, not a separate block — they're in IC1_A's loop). `Stage1::ClipMode` {Linear,Soft,Medium,Hard}; Soft=2 antiparallel 1N4148 pairs (one DiodePairT w/ 2*Is), Medium=one pair (Is), Hard=single DiodeT (asymmetric). 1N4148 Is=2.52e-9/Vt=25.85e-3/nDiodes=1.752 (nDiodes folds ideality into Vt — verified vs chowdsp header). Decomposition: diodes clamp Vf only (Ig=Vin/Zg unaffected by ideal op-amp). `tests/ClippingStage_Sine.cpp` validates each mode independently: small-signal linearity <0.2%, large-signal compression (±0.9V vs 76V linear), Soft/Medium symmetric, Hard asymmetric, all finite, dsp-validator PASS. Stage 1 linear also validated (`tests/Stage1_FreqResponse.cpp`, <0.04dB vs analytic). 9V SUPPLY MODELLED (2026-06-16): verified from schematic power section — straight single 9V, NO step-up/charge-pump (D7 1N5817 → R8 100r/C7 filter → op-amp V+ ~8.3V; VREF ~4.6V via R9/R10 8k2/10k). JRC4559 output headroom modelled as asymmetric soft saturation in Stage1.h (railClip: +2.2V/-3.1V bipolar, linear below 80% of rail; setRailVoltages/setRailClampEnabled to tune; refine at Step 9). Resolves the Hard-mode -77V. CARRY-FORWARD to Step 7: (a) DRIVE-pot node_F wiper tap unresolved — Stage1 outputs node_D; (b) apply A-taper R_max BASS=50k, DRIVE=1M in processBlock; (c) IC1_B (Stage 2) rails not yet modelled — add if it clips at -12dBu. Stages 0 & smoke test done. Step 2: AU passes auval. VST3 deferred.**
